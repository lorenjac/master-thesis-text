...
A notorious problem with NVRAM is consistency in case of crashes. Due to the
...
through MOB, its store buffer must be flushed. On x86 architectures this can be
achieved with a store fence operation such as \code{sfence}.
...
An approach to prevent such inconsistencies is to mark relevant memory pages as
uncacheable but that could introduce considerable overhead for frequently used
data. A more popular approach is to evict cache lines programmatically whenever
necessary \cite{condit2009better, dulloor2014system, oukid2017data}. On x86
architectures this can be done with the \code{clflush} or \code{clwb}
...
retaining the designated cache line.
