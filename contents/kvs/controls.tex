...
similar to \ac{RCU} \cite{mckenney1998read} but differs in that it can manage
more than two versions of an item at a time.
...
databases and \ac{KVS} \cite{cahill2009serializable, wu2017empirical}. In the
...
before $t$ was invoked. The key to this behaviour is the definition of the
visibility property.
...
$t_{RO}$ corresponds the the exact opposite serial ordering as do those of
$t_1$ and $t_2$.
...
The previous section introduced the \ac{MVCC} protocol \ac{SI}. \ac{SI} provides
...
section presents an overview on approaches to making serializing \ac{MVCC}
affordable.
...
to \ac{KVS} since they operate through ad-hoc queries instead of query
languages.
...
days of \ac{MVCC} \cite{kung1981optimistic}, but received new interest lately.
