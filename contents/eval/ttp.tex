...
This scenario simulates high contention through a small database. The expected
...
both Midas variants perform significantly worse than Echo, which can be
...
for low-contention environments, in the first place. However, it must be noted,
that other factors may play a role. For instance, Echo and Midas use different
backends to handle potential NVRAM. Also, the optimized variant of Midas does
not synchronize access to its index, which is a very optimistic approximation of
a non-blocking concurrent implementation.
