...
NVRAM. The store component of Midas is largely congruent to Figure
...
%
%         \node[initial,state] (A)                    {$Active$};
%         \node[state]         (B) [right of=A]       {$Committed$};
%         \node[state]         (C) [below right of=A] {$Failed$};
%
%         \path (A)   edge     node {commit}          (B)
%                     edge     node {abort, conflict} (C);
