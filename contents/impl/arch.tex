...
\begin{itemize}
    \item read: \code{read(<tx>, <key>, <value>)}
...
The update operation in Midas is called \code{write()}. It can be used to update existing items and insert new items. Because there can be no duplicates, Midas can always determine which action to perform. A pseudo code example of how \code{write()} is implemented is given in Algorithm \ref{alg:update}. As before, it is first checked whether the specified item has already been modified in the context of the current transaction \code{tx}. If so, the change set is modified accordingly and return. In case the change entry indicates that the item was previously removed by \code{tx}, the entry mode is changed from \code{mode::remove} to \code{mode::update}. This is possible because, due to the two-level store architecture, all updates and deletions are buffered until commit. If the target item is not in the change set, Midas proceeds as usual and looks up the items version history.
...
Unlike for reads, a missing history is not a reason to abort but indicates that a new version must be inserted. If a history exists, then a writable snapshot is determined. Note that there is no insertion if \code{getWriteSnapshot()} fails because that indicates a write-write conflict. In this case, the most suitable version has been invalidated with a TID of a transaction other than the current \code{tx}. Therefore, \code{tx} must abort according to the first-updater-wins principle shown in Section \ref{ch:concept-kvs-tx}. If a valid snapshot is found, its end timestamp is atomically replaced with the TID of \code{tx}. Finally, a new change set entry is created and the operation is complete. Note that, apart from replacing the snapshots timestamp, no NVRAM is updated at this point.
...
In order to manifest the changes of a transaction, the programmer must invoke \code{Store::commit()} to manifest its changes in the store. Instead of committing, the transaction may be aborted with \code{Store::abort()} at any time.
