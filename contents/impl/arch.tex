...
Unlike other \ac{KVS}, Midas can only be operated through transactions. Transactions that are either terminated or belong to another store instance are rejected. Once a transaction has been started, it can be used to modify the store with the following operations:
...
% TODO how is visibility determined (depending on the operation)
% TODO what facilities do that

\begin{algorithm}
\begin{algorithmic}[1]
\Procedure{isVisible}{tx, begin, end}
\If {$\text{isTID}(\textit{begin})$}
\State $\textit{otherTx} \gets \text{getTx}(\textit{begin})$
\If {$\text{getStatus}(\textit{otherTx}) \neq \text{Committed} \lor \text{getEnd}(\textit{otherTx}) > \text{getBegin}(\textit{tx})$}
...
\State $\textit{otherTx} \gets \text{getTx}(\textit{end})$
\If {$\text{getStatus}(\textit{otherTx}) = \text{Committed} \land \text{getEnd}(\textit{otherTx}) < \text{getBegin}(\textit{tx})$}
\State \Return $\text{false}$
\EndIf
\Else
